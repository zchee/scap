...
| `scap.listCache` / `SCAP_LIST_CACHE` env and `scap list --cache` / `--no-cache` / `--cache-check` | An opt-in index that answers `scap list` by `stat`ing the tree instead of reading it. One file per root under `$XDG_CACHE_HOME/scap` remembers what the last walk found and the mtime each directory had; a directory whose mtime has not moved has the same entry list, so its `.git` presence and its child set are unchanged and the recorded answer stands. Only what changed is re-walked, and the printed bytes are identical either way. **Do not enable it on a network filesystem, or on any filesystem whose mtime granularity is 1 s or coarser.** Attribute caching over NFS can report a stale mtime for a directory that has changed, and a coarse timestamp cannot distinguish two changes inside one tick; either makes the index miss a repository. Locally it stays blind to three things, none of which moves any directory's mtime: a deliberately rewound timestamp (`touch -t`); a `.git` appearing or disappearing inside a *symlinked* repository; and a directory left searchable but not readable (`chmod 0111`), where `stat` still walks through while `readdir` is refused, so the index keeps reporting repositories the walk can no longer enumerate. `--cache-check` walks afresh, prints the difference on stderr and exits 1, which is how any of those is settled; it also rewrites the index from that fresh walk, so a run that reports a difference repairs it. It cannot be combined with `--no-cache` — checking against an index the same command forbids reading has no meaning, so the pair is a usage error rather than a silent precedence. Among the three ways of *enabling* the index, `--no-cache` beats `--cache`, which beats `SCAP_LIST_CACHE`, which overrides `scap.listCache` as `SCAP_LIST_EXCLUDE` overrides `scap.listExclude`. A walk that could not read some directory is never written down as an index, and any unreadable or unrecognised index file is discarded and rebuilt rather than failing the listing. Off by default, and ghq has no equivalent. (ADR-10, ADR-13) |
...
| A repository directory that is readable but not searchable is listed, and the listing still exits 0 | At mode 0444 a directory can be read but not `stat`ted through, so `<dir>/.git` cannot be probed from the parent; the walk opens the directory instead — which needs only the read bit — and finds `.git` among the entries it has already read, under both detection strategies — provided the directory listing types that entry, as APFS, ext4 and btrfs do; where it does not (overlayfs, for one), the fallback `stat` needs the search bit this directory lacks and the repository is not listed. There is no ghq behaviour to copy here, as there is none for a root whose `stat` fails oddly: ghq aborts the *whole* listing on such a directory with `failed to filter repos while walkLocalRepositories(repo): permission denied`, exits 1 and prints nothing at all, so one directory's mode costs every other repository on the machine. scap prints the repository without warning about it. (ADR-9, ADR-13) |
